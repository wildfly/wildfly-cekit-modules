...
maven_s2i_save_artifacts > $HOME/m2-incremental.tar

TAR_CONTENT=$HOME/tar-content
mkdir -p "$TAR_CONTENT"

# add maven artifacts if any
if [ -s $HOME/m2-incremental.tar ]; then
  pushd "$TAR_CONTENT" &> /dev/null
  tar -xf $HOME/m2-incremental.tar &> /dev/null
  popd &> /dev/null
fi
...
rm -rf $HOME/m2-incremental.tar $TAR_CONTENT &> /dev/null
